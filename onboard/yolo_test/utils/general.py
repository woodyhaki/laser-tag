...
def nms(boxes, scores, iou_threshold):
    """
    纯PyTorch实现的非极大值抑制 (NMS)
    
    参数:
        boxes (Tensor): [N, 4] 格式的边界框，格式为 (x1, y1, x2, y2)
        scores (Tensor): [N] 每个边界框的置信度分数
        iou_threshold (float): IoU阈值，高于此值的框会被抑制
    ...
    while indices.numel() > 0:
        ...
        current_box = boxes_sorted[current_idx]
        remaining_boxes = boxes_sorted[1:]
        
        # 3. 计算当前框与所有剩余框的IoU
        xx1 = torch.max(remaining_boxes[:, 0], current_box[0])
        yy1 = torch.max(remaining_boxes[:, 1], current_box[1])
        xx2 = torch.min(remaining_boxes[:, 2], current_box[2])
        yy2 = torch.min(remaining_boxes[:, 3], current_box[3])
        ...
        boxes_sorted = remaining_boxes[mask]
        areas = areas[1:][mask]  # 注意：areas也需要更新
